...
@Model
final class ProjectSubject {
    @Attribute(.unique) var id: UUID
    @Attribute(.unique) private(set) var slug: String
    @Relationship(deleteRule: .nullify, inverse: \ProjectDoc.subjects)
     var docs: [ProjectDoc] = []
    ...
    private static func slugExists(_ slug: String, in context: ModelContext) throws -> Bool {
        let fd = FetchDescriptor<ProjectSubject>(
            predicate: #Predicate<ProjectSubject> { $0.slug == slug }
        )
