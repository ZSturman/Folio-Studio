...
@Model
final class ProjectMedium {
    @Attribute(.unique) var id: UUID
    @Attribute(.unique) private(set) var slug: String
    @Relationship(deleteRule: .nullify, inverse: \ProjectDoc.mediums)
     var docs: [ProjectDoc] = []
    ...
    private static func slugExists(_ slug: String, in context: ModelContext) throws -> Bool {
        let fd = FetchDescriptor<ProjectMedium>(
            predicate: #Predicate<ProjectMedium> { $0.slug == slug }
        )
