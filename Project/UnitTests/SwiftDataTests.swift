...
// MARK: - Path Export Tests

@Suite("SwiftData Path Export Tests")
struct PathExportTests {
    
    @Test("Export formats generate valid output")
    func testExportFormats() throws {
        let schema = Schema([ProjectDoc.self, ProjectDomain.self])
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: schema, configurations: [config])
        let context = ModelContext(container)
        
        // Create test projects
        let doc1 = ProjectDoc(title: "Project 1", filePath: "/path/to/project1.folioDoc")
        let doc2 = ProjectDoc(title: "Project 2", filePath: "/path/to/project2.folioDoc")
        ...
        #expect(projects.contains { $0.title == "Project 1" })
        #expect(projects.contains { $0.filePath.contains("project1.folioDoc") })
    }
    
    @Test("File path validation detects missing files")
    func testFilePathValidation() throws {
        let schema = Schema([ProjectDoc.self])
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: schema, configurations: [config])
        let context = ModelContext(container)
        
        let doc = ProjectDoc(title: "Test", filePath: "/nonexistent/path.folioDoc")
        context.insert(doc)
        try context.save()
        
        // Validate file doesn't exist
        let exists = FileManager.default.fileExists(atPath: doc.filePath)
        #expect(exists == false)
    }
    
    @Test("Project deletion removes from context")
    func testProjectDeletion() throws {
        ...
        let doc = ProjectDoc(title: "ToDelete", filePath: "/tmp/test.folioDoc")
        context.insert(doc)
        try context.save()
        
        var descriptor = FetchDescriptor<ProjectDoc>()
        ...
        descriptor = FetchDescriptor<ProjectDoc>()
        results = try context.fetch(descriptor)
        #expect(results.count == 0)
    }
    
    @Test("Query returns projects sorted by updatedAt")
    func testProjectQuerySorting() throws {
        let schema = Schema([ProjectDoc.self])
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: schema, configurations: [config])
        let context = ModelContext(container)
        
        let old = ProjectDoc(title: "Old", filePath: "/old.folioDoc")
        old.updatedAt = Date(timeIntervalSinceNow: -3600) // 1 hour ago
        
        let new = ProjectDoc(title: "New", filePath: "/new.folioDoc")
        new.updatedAt = Date() // now
