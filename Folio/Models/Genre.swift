...
@Model
final class ProjectGenre {
    @Attribute(.unique) var id: UUID
    @Attribute(.unique) private(set) var slug: String
    @Relationship(deleteRule: .nullify, inverse: \ProjectDoc.genres)
     var docs: [ProjectDoc] = []
    ...
    private static func slugExists(_ slug: String, in context: ModelContext) throws -> Bool {
        let fd = FetchDescriptor<ProjectGenre>(
            predicate: #Predicate<ProjectGenre> { $0.slug == slug }
        )
