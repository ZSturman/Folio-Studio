...
@Model
final class ProjectTopic {
    @Attribute(.unique) var id: UUID
    @Attribute(.unique) private(set) var slug: String
    @Relationship(deleteRule: .nullify, inverse: \ProjectDoc.topics)
     var docs: [ProjectDoc] = []
    ...
    private static func slugExists(_ slug: String, in context: ModelContext) throws -> Bool {
        let fd = FetchDescriptor<ProjectTopic>(
            predicate: #Predicate<ProjectTopic> { $0.slug == slug }
        )
